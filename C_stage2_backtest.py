"""
Stage 2 - Trailing Stop Backtest for C (Citigroup Inc.)
Methodology: 3 years historical data, ATR-derived stop levels, Parkinson
             volatility estimator, 10,000 Monte Carlo sims.

Stop levels are computed as multiples of ATR(14) rather than fixed percentages.
Multipliers tested: 1.0×, 1.5×, 2.0×, 2.5×, 3.0× ATR.
High-vol cap: if ATR% of price > 5%, multiplier range is halved (1×ATR max).
Time backstop: in the Monte Carlo simulation a position that has not gained
>= 0.5% after 5 trading days is closed (thesis-not-working rule).
"""
import math
import numpy as np
import pandas as pd
...
LOOKBACK_DAYS = 3 * 365
N_SIMS        = 10000
CAPITAL       = 1000      # normalised position size
HOLD_DAYS     = 252       # 1 trading year forward simulation
ATR_PERIOD    = 14
# Multipliers to test — actual stop % is multiplier × ATR%
# Rule: if ATR% of price > 5% (high-vol stock), cap multiplier at 1.0
ATR_MULTIPLIERS     = [1.0, 1.5, 2.0, 2.5, 3.0]
ATR_HIGHVOL_CAP_PCT = 5.0    # ATR% threshold for high-vol treatment
ATR_HIGHVOL_MULT    = 1.0    # multiplier cap when high-vol
MAX_STOP_PCT        = 0.08   # hard cap: stop never wider than 8%
# Time backstop
BACKSTOP_DAYS       = 5      # trading days
BACKSTOP_THRESHOLD  = 0.005  # must be >= +0.5% gain on day 5 or exit
...
# ── Parkinson volatility estimator (uses High/Low — more accurate than Close-only)
raw['park_var'] = (1.0 / (4.0 * math.log(2))) * (np.log(raw['High'] / raw['Low'])) ** 2
sigma = float(np.sqrt(raw['park_var'].mean()))   # daily vol from intraday range

# Mean daily return still from Close (directional signal)
raw['ret'] = raw['Close'].pct_change()
raw = raw.dropna()
mu  = float(raw['ret'].mean())

# ── ATR(14) from True Range (High/Low/PrevClose) ──────────────────────────────
...
atr_pct         = current_atr / current_price          # ATR as fraction of price

# Select multiplier range based on high-vol rule
...
# Convert multipliers → stop percentages (capped at MAX_STOP_PCT)
...
for mult, stop_pct in zip(multipliers_to_test, STOP_LEVELS):
    ...
    for _ in range(N_SIMS):
        ...
        for day in range(HOLD_DAYS):
            ...
            # Time backstop: exit if not >= +0.5% after 5 trading days
            if days_in_trade == BACKSTOP_DAYS:
                gain = (cap - CAPITAL) / CAPITAL
                if gain < BACKSTOP_THRESHOLD:
                    cap   = CAPITAL
                    peak  = CAPITAL
                    stops += 1
                    days_in_trade = 0
...
print('\n[4/4] RESULTS')
print('=' * 70)
print(f'STAGE 2 BACKTESTING RESULTS -- {TICKER} (Citigroup Inc.)')
print('=' * 70)
...
if best_label:
    ...
    print()
    if passes:
        print(f'[PASS] {TICKER} PASSES STAGE 2 -- proceed to Stage 3 (Sentiment Analysis)')
    else:
        print(f'[FAIL] {TICKER} FAILS STAGE 2 -- criteria not fully met')
else:
    print('\n[FAIL] No stop level meets Sharpe > 0.3 and P(Loss>5%) < 5%')
    print(f'[FAIL] {TICKER} FAILS STAGE 2')
...
json_path = 'Analysis_Outcomes/C_stage2_backtest.json'
...
txt_path = f'Analysis_Outcomes/C_stage2_{status}.txt'
with open(txt_path, 'w', encoding='utf-8') as f:
    f.write(f'C - STAGE 2 {"PASSED" if passes else "FAILED"}\n')
    f.write(f'Date: {datetime.now().strftime("%Y-%m-%d")}\n')
    f.write(f'Decision: {"Proceed to Stage 3 (Sentiment Analysis)" if passes else "Do not proceed"}\n\n')
    f.write('METHODOLOGY (UPDATED — ATR-BASED STOPS):\n')
    f.write(f'- Data: 3 years of C daily OHLCV history ({len(raw)} trading days)\n')
    f.write(f'- Price range over 3 years: ${raw["Close"].min():.2f} - ${raw["Close"].max():.2f}\n')
    ...
    f.write(f'- Stop levels tested: ATR multipliers {multipliers_to_test}\n')
    f.write(f'- Time backstop: Day {BACKSTOP_DAYS}, min gain +{BACKSTOP_THRESHOLD*100:.1f}%\n')
    f.write(f'- Simulations: {N_SIMS:,} Monte Carlo runs per stop level\n\n')
    f.write('COMPARISON TABLE:\n')
    f.write(f'{"Stop Level":<24} {"Sharpe":<10} {"Mean Ret":<12} {"P(Loss>5%)":<14} {"Stops/Yr":<10} {"P(Profit)"}\n')
    for label, m in results.items():
        mark = '  <-- RECOMMENDED' if label == best_label else ''
        f.write(f'{label:<24} {m["sharpe"]:<10.3f} {m["mean_ret_pct"]:>+9.2f}%  '
                f'{m["p_loss_gt5"]:>11.2f}%  {m["avg_stops_yr"]:>8.1f}   '
                f'{m["p_profit"]:>7.1f}%{mark}\n')
    if best_label:
        bm = results[best_label]
        f.write(f'\nRECOMMENDED TRAILING STOP: {best_label}\n\n')
        f.write('PERFORMANCE AT RECOMMENDED STOP:\n')
        f.write(f'- Sharpe Ratio       : {bm["sharpe"]:.3f}  [PASS: > 0.3 required]\n')
        f.write(f'- Mean Annual Return : {bm["mean_ret_pct"]:+.2f}%\n')
        f.write(f'- P(Loss > 5%)       : {bm["p_loss_gt5"]:.2f}%  [PASS: < 5.0% required]\n')
        f.write(f'- Expected Stops/Yr  : {bm["avg_stops_yr"]}\n')
        f.write(f'- P(Profit)          : {bm["p_profit"]:.1f}%\n\n')
        f.write(f'STAGE 2 DECISION: {"PASS" if passes else "FAIL"}\n')
        if passes:
            f.write('NEXT STEP: Stage 3 -- Sentiment Analysis\n')
            f.write('  Required: Download Morningstar analyst report for C from Merrill Lynch\n')
            f.write('  Save to: stock-trading-system/sentiment_analyzer/Analyst_reports/Analyst_C.pdf\n')
            f.write('  Run: python main.py --ticker C --pdf-path "Analyst_reports/Analyst_C.pdf"\n')
