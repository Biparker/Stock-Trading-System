"""
Stage 2 - Trailing Stop Backtest for META (Meta Platforms, Inc.)
...
print('\n[4/4] RESULTS')
print('=' * 70)
print(f'STAGE 2 BACKTESTING RESULTS -- {TICKER} ({COMPANY})')
print('=' * 70)
...
json_path = 'Analysis_Outcomes/META_stage2_backtest.json'
...
txt_path = f'Analysis_Outcomes/META_stage2_{status}.txt'
with open(txt_path, 'w', encoding='utf-8') as f:
    f.write(f'META - STAGE 2 {"PASSED" if passes else "FAILED"}\n')
    ...
    f.write(f'- Data: 3 years of META daily OHLCV history ({len(raw)} trading days)\n')
    ...
    if best_label:
        ...
        if passes:
            ...
            f.write('  Required: Download Morningstar analyst report for META from Merrill Lynch\n')
            f.write('  Save to: stock-trading-system/sentiment_analyzer/Analyst_reports/Analyst_META.pdf\n')
            f.write('  Run: python main.py --ticker META --pdf-path "Analyst_reports/Analyst_META.pdf"\n')
